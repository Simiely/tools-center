...
### 功能
- **应用管理编辑弹窗:link 型新增「链接」输入框**(v0.12.6):编辑快捷方式时可直接修改跳转目标 url,保存即写回 tool.json;app 型不显示该行
- 后端  白名单新增 (仅 link 型接受,校验 http(s):// 前缀,app 型传 url 报错)
-  返回 link 型  字段供编辑弹窗回填

### 验证
- 新增 tests/meta-link.test.mjs(4 例:改 url 写回/非法 url 拦截/app 型拒绝/scanDisk 回填)
- 全量 80/80 通过;端到端:创建 link → 编辑 url → 写回确认 → 非法 url 400 拦截

## v0.12.5 (2026-08-08) · 单能力 Tab 自动隐藏(能力是环境需求,不是业务分类)

### 界面
- **能力 Tab 单能力时自动隐藏**(2026-08-08):顶部筛选 Tab 由「全部 + 分类 + 能力」构成——当所有工具只声明同一种能力(如都声明 `storage`)时,能力 Tab 只剩 1 个、与「全部」冗余且失去筛选意义;现仅当能力种类 ≥2 时才渲染能力 Tab;同时重置已选中的能力筛选
- 工具卡片/详情弹层的能力徽标(`cap-chip`/`capLabel`)**不受影响**,仍照常显示
- 参考场景:gh-release-center 等工具统一声明 `capabilities:["storage"]` 后,顶部不再出现孤立的「💾 存储」Tab
