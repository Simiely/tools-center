...
>
> **架构定位先看**:[`docs/架构梳理.md`](docs/架构梳理.md)——主干框架(9 文件,去掉无法托管)vs 附加模块(接入/存储/备份/安全/能力)。改代码前先判断动的是主干还是附加,主干改动必须跑全量测试。
...
1. **工具接入契约**:一切以 `tool.json` 为准;`type: app` 才 spawn 托管,`type: link` 只做卡片跳转(302),不托管;**放目录+tool.json → 刷新页面即自动发现并启动**(`GET /api/tools` 访问即重扫)
2. **端口规划**:主程序 8080,工具段 8100~8199;启动时校验冲突,冲突工具标记 error 跳过
3. **崩溃拉起必须退避**:指数退避 1s→30s 封顶,连续失败标记 error,防"崩溃风暴";**运行状态存 manager 内部与 spec 解耦**(重扫不丢状态)
