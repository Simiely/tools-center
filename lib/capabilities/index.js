...
export function listCapabilities() {
  return [...registry.entries()].map(([name, mod]) => ({
    name,
    status: mod.status ? mod.status() : CAP_STATUS.IDLE,
  }));
}

// ---------- 内置空壳(骨架:声明接口,实现由各自里程碑填充) ----------
...
// 空壳能力:browser / storage / network(实现 M1/M2 填充)
...
export function initCapabilities() {
  for (const name of ["browser", "storage", "network"]) {
