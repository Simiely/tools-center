...
import { classifyDirFiles } from "../core/disk-ops.js"; // 数据识别(升级保留数据,2026-08-06)
 ...
 */
export async function zipToTool(data, filename = "tool.zip") {
  ...
  try {
    ...
    manager.sync();
    return { code: 201, body: { ok: true, created: !existed, tool: publicTool(t) } };
  } catch (e) {
 ...
 */
export function createZipImportTask(url) {
  ...
  (async () => {
    try {
      ...
      task.progress = 55;
      const out = await zipToTool(buf, name);
      if (!out.body || !out.body.ok) throw new Error(out.body?.error || "导入失败");
      ...
      if (contentType.includes("multipart/form-data")) {
        try {
          ...
          if (!target) {
            ...
            const out = await zipToTool(f.data, f.filename);
            return sendJson(res, out.code, out.body);
          }
