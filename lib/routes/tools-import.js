// lib/routes/tools-import.js - 在线导入域路由（Git 仓库 / zip 链接，异步任务 + 进度轮询）
// v0.13.0 从 tools-files.js 拆出：与文件上传(/api/files)按职责分离，日志读取独立为 tools-logs.js。
// 本组挂 module:"import"（可关）；关闭后 /api/tools/import 与 /api/tools/import/status 均 404。
// 核心业务：zipToTool（纯 zip 零输入创建/覆盖工具，/api/files 与链接导入共用）。
...
import { unzipAsync, findManifest, checkZipBuffer } from "../core/upload.js";
import { passOk } from "../core/auth.js"; // 高危写面密码门(2026-08-06 审计加固)
import { classifyDirFiles } from "../core/data-classify.js"; // 数据识别公共层(升级保留数据)
import { importFromGit } from "../core/git.js"; // Git 仓库导入
...
export const toolsImportRoutes = [
  // 在线导入(Git 仓库 / zip 链接,异步任务 + 进度轮询)
  {
