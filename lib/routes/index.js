...
// 域拆分:tools-proxy(反代) / tools-crud(工具增删查) / tools-files(上传) / tools-import(在线导入)
//         / tools-logs(日志,主干) / tools-prefix(单工具操作)
...
// v0.13.0:日志读取独立为主干组(不再随 import 模块关闭);导入业务(上传+在线导入)归 import 模块
...
import { toolsImportRoutes } from "./tools-import.js";
import { logsRoutes } from "./tools-logs.js";
...
import { uiOrderRoutes } from "./ui-order.js";
...
const GROUPS = [
  ...
  { routes: toolsFilesRoutes, module: "import" }, // 附加:文件/zip 上传
  { routes: toolsImportRoutes, module: "import" }, // 附加:Git 仓库 / zip 链接在线导入
  { routes: logsRoutes },                // 主干:工具日志读取(不随 import 关闭)
  ...
  { routes: uiOrderRoutes },             // 主干:UI 排序偏好(分组/卡片顺序,拖动保存)
];
