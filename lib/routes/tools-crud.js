// lib/routes/tools-crud.js - 工具 CRUD 路由:列表/创建/删除/校验/重载/恢复托管
// 职责:工具注册表的增删查操作(不涉及进程控制,进程由 manager 在业务后同步)。
// 注意:在线导入(Git 仓库 / zip 链接)路由已迁至 tools-files.js(import 模块,2026-08-06)。
import { sendJson, jsonBody, publicTool, refreshTools } from "./helpers.js";
import { scanTools, listTools, getTool, createTool, removeTool, validateManifest, updateToolMeta } from "../core/registry.js";
import { restoreTool } from "../core/lifecycle.js";
import { fetchFavicon } from "../core/favicon.js";
import * as manager from "../core/manager.js";
import { checkPass, passOk } from "../core/auth.js";
...
export const toolsCrudRoutes = [
  ...
  },
  {
    ...
    handler: async (req, res) => {
      ...
      // 密码门(v0.13.0 审计补漏):创建工具可携带任意 cmd = 执行任意命令,设置密码后必须校验。
      // 与 import/files 同款:未设置密码时仍放行(内网单用户默认),设置后拦截无凭证请求。
      if (!passOk(spec, req.headers)) return sendJson(res, 403, { ok: false, error: "需要管理员密码", needAuth: true });
      try {
  ...
  // 自动抓取网站图标(添加 link 快捷方式时调用,v0.12.6):目标 url → 解析首页 <link rel=icon> 或兜底 /favicon.ico
  {
    m: "GET", p: "/api/favicon",
    handler: async (req, res, url) => {
      const target = url.searchParams.get("url") || "";
      if (!/^https?:\/\//.test(target)) return sendJson(res, 400, { ok: false, error: "需要 http(s):// 目标地址" });
      try {
        const icon = await fetchFavicon(target);
        return sendJson(res, 200, { ok: true, icon });  // icon 可为 null(抓不到,前端静默降级)
      } catch (e) { return sendJson(res, 200, { ok: true, icon: null, error: e.message }); }
    },
  },
  {
    m: "POST", p: "/api/reload",
  ...
  // 应用管理(v0.12.2):更新工具显示信息(名称/图标/分组/描述),写回声明文件后重扫
  {
    m: "POST", p: "/api/tools/meta",
    handler: async (req, res) => {
      let b;
