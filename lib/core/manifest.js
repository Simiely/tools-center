...
const KNOWN_CAPABILITIES = ["browser", "storage", "network"]; // 已注册能力(与 lib/capabilities/index.js 保持一致)
...
export function normalizeManifest(raw, fallbackId) {
  ...
  m.unknownCaps = m.capabilities.filter((c) => !KNOWN_CAPABILITIES.includes(c));
  return m;
}
