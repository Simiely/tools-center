...
function start(tool) {
  ...
  run.set(tool.id, rec);
  rec.stopped = false; // 恢复/重启时清除手动停止标记——否则该进程崩溃后 exit 回调见 stopped 直接 return,不再自动拉起(2026-08-06 审计发现)
  rec.status = "starting";
  rec.error = "";
  ...
  try {
    child = spawn(exe, args, {
      ...
      // 平台托管的工具一律注入 WB_COLLECTOR=file(文件/WebDAV 采集,不采集 cookie):
      // 否则工具进程在 Windows 上默认按桌面版(edge)运行,启动时自动弹浏览器
      // (如积分工具 wb-gui.mjs 的 listen 回调)。tool.env 可显式覆盖。
      env: { ...process.env, WB_COLLECTOR: "file", ...tool.env },
...
async function healthCheck() {
  // v0.13.0:并行探测(此前串行 for-await,N 个 down 工具最坏耗时 N×3s,状态刷新严重滞后)
  await Promise.all(listTools().map(async (t) => {
    if (t.type !== "app" || !t.valid || !t.health) return;
    const rec = run.get(t.id);
    if (!rec || !rec.proc) { if (rec) rec.health = "down"; return; }
