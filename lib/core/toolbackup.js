...
/** 清理旧工具备份 zip:只保留最近 BACKUP_KEEP 份(2026-08-13,防备份无限累积撑满磁盘) */
function pruneToolBackups() {
  try {
    const files = fs.existsSync(BACKUPS_ROOT)
      ? fs.readdirSync(BACKUPS_ROOT).filter((n) => n.startsWith(TOOLBACKUP_PREFIX) && n.endsWith(".zip")).sort()
      : [];
    for (const f of files.slice(0, Math.max(0, files.length - CONFIG.BACKUP_KEEP))) {
      try { fs.unlinkSync(path.join(BACKUPS_ROOT, f)); } catch {}
    }
  } catch {}
}
...
export function backupTools() {
  ...
  pruneToolBackups(); // 只保留最近 N 份(2026-08-13)
...
/**
 * 删除指定备份 zip。
 * 安全:仅允许 data/backups/ 下的 tools-*.zip(防路径穿越),文件不存在视为已删(幂等)。
 * @param {string} name 备份文件名(如 tools-2026-08-04T...-zip)
 * @returns {{deleted: boolean}}
 */
export function deleteToolBackup(name) {
  const n = String(name || "").trim();
  if (!n || !n.startsWith(TOOLBACKUP_PREFIX) || !n.endsWith(".zip") || n.includes("/") || n.includes("\\") || n !== path.basename(n)) {
    throw new Error("非法备份文件名");
  }
  const file = path.join(BACKUPS_ROOT, n);
  if (!fs.existsSync(file)) return { deleted: false }; // 幂等:不存在视为已删
  fs.unlinkSync(file);
  return { deleted: true };
}

/** 清理 tools/ 下同 id 的旧 .pre-restore-* 残留目录(2026-08-13:多次恢复会堆积无 manifest 的幽灵目录) */
function cleanPreRestoreDirs(id) {
  try {
    if (!fs.existsSync(DIRS.tools)) return;
    const prefix = id + ".pre-restore-";
    for (const name of fs.readdirSync(DIRS.tools)) {
      if (name.startsWith(prefix)) {
        try { fs.rmSync(path.join(DIRS.tools, name), { recursive: true, force: true }); } catch {}
      }
    }
  } catch {}
}
...
export function restoreFromZip(zipName, ids) {
  ...
  for (const id of restoreIds) {
    ...
    // 目标目录已存在 → 备份现有;先清理同 id 旧 .pre-restore-* 残留(2026-08-13)
    const dest = path.join(DIRS.tools, id);
    if (fs.existsSync(dest)) {
      cleanPreRestoreDirs(id);
