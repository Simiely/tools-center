// lib/proxy.js - 反向代理(仅 app 型)+ link 型 302 跳转 + WebSocket 升级
// 零依赖:node:http 转发,流式透传,60s 超时;WebSocket 经 upgrade 事件双向转发。
    ...
    (prs) => {
      ...
      if (ct.includes("text/html")) {
        ...
        prs.on("end", () => {
          ...
          delete headers["transfer-encoding"];  // 已重写 body + 显式 content-length,须移除上游 chunked(否则 HTTP/1.1 非法头组合,严格客户端拒绝)
...
/**
 * WebSocket 升级代理:转发 /tool/<id>/... 的 upgrade 到工具端口。
 * @returns {boolean} true = 已处理(含 404),false = 不该由本层处理
 */
export function proxyUpgrade(req, socket, head, url) {
  const seg = url.pathname.split("/"); // ["", "tool", id, ...rest]
  if (seg.length < 3 || seg[1] !== "tool") return false;
  const id = decodeURIComponent(seg[2]);
  const tool = getTool(id);
  if (!tool || tool.type !== "app" || !tool.valid) {
  ...
  const pr = http.request({
    ...
    path: target.pathname + target.search,
    method: "GET",
    headers: { ...req.headers, host: `127.0.0.1:${tool.port}`, connection: "Upgrade", upgrade: "websocket" },
  });
  pr.on("upgrade", (prs, psocket, phead) => {
    socket.write(`HTTP/1.1 101 Switching Protocols\r\n${Object.entries(prs.headers).map(([k, v]) => `${k}: ${v}\r\n`).join("")}\r\n`);
    psocket.pipe(socket);
    socket.pipe(psocket);
  });
  pr.on("error", () => { try { socket.destroy(); } catch {} });
  pr.end();
  return true;
}
