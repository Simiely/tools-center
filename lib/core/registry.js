...
// 已解除托管的工具 id(挂载型工具目录删除失败 EBUSY 时记录;扫描时跳过,避免重扫又识别回来)
// 持久化在 data/removed-tools.json(平台数据目录,挂载卷),删除挂载工具不丢记录
...
export function scanTools() {
  ...
  if (fs.existsSync(DIRS.tools)) {
    for (const name of fs.readdirSync(DIRS.tools)) {
      ...
      if (!isDir) continue;
      if (removedSet.has(name)) continue;  // 已解除托管(挂载型工具),重扫跳过
 ...
 * @returns {{removed: boolean, dirKept: boolean}} dirKept=true 表示目录保留(挂载型工具)
 */
