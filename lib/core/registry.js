...
let byDir = new Map();  // dir(目录名) -> ToolSpec(scanDisk 按目录名查,2026-08-09 走查 P-1:目录名≠id 时编辑/管理失效)
...
export function scanTools() {
  ...
  map = found;
  byPort = ports;
  // 目录名索引(与 id 可能不同:手动放置时目录名只是目录,id 来自声明文件)
  byDir = new Map();
  for (const spec of map.values()) if (spec.dir) byDir.set(path.basename(spec.dir), spec);
  return map;
}
...
/** 按目录名查工具(scanDisk 等磁盘管理用;目录名≠id 时仍可定位,2026-08-09 P-1) */
export function getToolByDir(dir) { return byDir.get(dir) || null; }
...
/** 更新工具显示信息(应用管理:v0.12.2 —— 名称/图标/分组/描述)。
 * 只改「显示在外面的内容」,不动 cmd/port/entry 等运行配置;写回声明文件后重扫注册表。
 * 声明文件与 loadManifest 一致:优先 manifest.json,否则 tool.json。
 * @param {string} id 工具 id
 * @param {{name?:string, icon?:string, group?:string, desc?:string}} patch 要更新的字段(白名单)
 * @returns {object} 更新后的 ToolSpec
 */
export function updateToolMeta(id, patch) {
  const t = getTool(id);
  if (!t || !t.valid) throw new Error("工具不存在或配置无效: " + id);
  const file = fs.existsSync(path.join(t.dir, "manifest.json"))
    ? path.join(t.dir, "manifest.json")
    : path.join(t.dir, "tool.json");
  if (!fs.existsSync(file)) throw new Error("缺少声明文件: " + file);
  const spec = readJson(file);
  if (!spec || typeof spec !== "object") throw new Error("声明文件解析失败");
  const n = {};
  if ("name" in patch) {
  ...
  if ("icon" in patch) n.icon = String(patch.icon || "").trim() || "🧰";
  if ("group" in patch) n.group = String(patch.group || "").trim() || "工具";
  if ("desc" in patch) n.desc = String(patch.desc || "").trim();
  // link 型专属:修改跳转目标 url(v0.12.6);app 型忽略 url 字段
  if ("url" in patch) {
    if (spec.type !== "link") throw new Error("仅 link 型(快捷方式)支持修改链接");
    const url = String(patch.url || "").trim();
    if (!/^https?:\/\//.test(url)) throw new Error("链接需 http(s):// 开头");
    n.url = url;
  }
  if (!Object.keys(n).length) throw new Error("没有可更新的字段");
  Object.assign(spec, n);
  fs.writeFileSync(file, JSON.stringify(spec, null, 2) + "\n", "utf8");
  scanTools();
  const nt = getTool(id);
  if (!nt || !nt.valid) throw new Error("更新后工具配置无效: " + (nt ? nt.error : "未知"));
  return nt;
}
