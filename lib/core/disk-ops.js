...
import { getTool, getToolByDir, scanTools } from "./registry.js";
...
export function scanDisk() {
  ...
  for (const name of fs.readdirSync(DIRS.tools)) {
    ...
    if (!isDir) continue;
    const t = getToolByDir(name) || getTool(name); // 目录名优先(id 可能≠目录名,2026-08-09 P-1)
    ...
    out.push({
      ...
      url: t && t.type === "link" ? (t.url || "") : "",  // link 型跳转目标(编辑弹窗回填用,v0.12.6)
