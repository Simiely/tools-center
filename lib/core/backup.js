...
import * as webdav from "../capabilities/storage/webdav.js";
...
/** 简单列出 WebDAV 目录文件:读上传时记录的 _filelist.json(依赖它,免 PROPFIND/XML) */
...
/** 简单列出 WebDAV 目录文件(逐 PROPFIND? 简化:依赖 manifest 里的文件名 —— 由快照不含文件名,改用占位:空实现)
 *  说明:WebDAV 目录列举需 PROPFIND,XML 解析零依赖较繁;当前下载依赖 manifest 中工具 id,
 *  文件清单通过上传时的 _filelist.json 记录(见 webdavUpload 扩展)。 */
