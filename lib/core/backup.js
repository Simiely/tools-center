...
/** 清理旧备份目录:只保留最近 BACKUP_KEEP 份(2026-08-13,防备份无限累积撑满磁盘) */
function pruneBackups() {
  try {
    const dirs = fs.existsSync(BACKUPS_ROOT)
      ? fs.readdirSync(BACKUPS_ROOT).filter((n) => { try { return fs.statSync(path.join(BACKUPS_ROOT, n)).isDirectory(); } catch { return false; } }).sort()
      : [];
    for (const d of dirs.slice(0, Math.max(0, dirs.length - CONFIG.BACKUP_KEEP))) {
      try { fs.rmSync(path.join(BACKUPS_ROOT, d), { recursive: true, force: true }); } catch {}
    }
  } catch {}
}
...
export function localBackup() {
  ...
  pruneBackups(); // 只保留最近 N 份(2026-08-13)
...
/** 路径穿越防护:把 parts 安全 join 到 root 内;结果逃逸 root 或含非法段返回 null(2026-08-06 审计加固) */
function safeJoin(root, ...parts) {
  const r = path.resolve(root);
  const dest = path.resolve(r, ...parts);
  if (dest !== r && !dest.startsWith(r + path.sep)) return null;
  return dest;
}
...
export async function webdavDownload(cfg, remoteTs) {
  const { url, user, pass } = cfg;
  // remoteTs 来自用户请求参数,只允许纯数字时间戳(防穿越到 BACKUPS_ROOT 之外)
  const ts = String(remoteTs || "").trim();
  if (!/^\d{10,14}$/.test(ts)) throw new Error("备份标识无效: " + remoteTs);
  const dir = `${webdav.BACKUP_DIR}/${ts}`;
  ...
  const dest = safeJoin(BACKUPS_ROOT, ts);
  if (!dest) throw new Error("备份标识越界");
  fs.mkdirSync(dest, { recursive: true });
  fs.writeFileSync(path.join(dest, "_manifest.json"), manifestRaw, "utf8");
  for (const t of manifest.tools || []) {
    // t.id / rel 来自云端 manifest(外部可控),必须落在恢复目录内
    const tdir = safeJoin(dest, String(t.id || ""));
    if (!tdir) { throw new Error("备份清单含非法工具路径: " + String(t.id)); }
    fs.mkdirSync(tdir, { recursive: true });
    ...
    for (const rel of files) {
      ...
      const to = safeJoin(tdir, String(rel || ""));
      if (!to) throw new Error("备份清单含非法文件路径: " + rel);
      fs.mkdirSync(path.dirname(to), { recursive: true });
      fs.writeFileSync(to, content);
    }
  }
  return { ts, dir: dest, tools: manifest.tools || [] };
}
