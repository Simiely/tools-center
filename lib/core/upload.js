...
/** unzip 退出码语义(Info-ZIP):0=成功;1=警告但已解压完成(典型:zip 条目名用反斜杠
 * \ 作路径分隔符的 Windows 风格打包——非标准但 unzip 会转换后正常解压,仅返回 1);
 * >=2=真错误(CRC 失败/不支持压缩方法/加密等)。
 * 2026-08-06 实测:gh-release-center 的 zip 用反斜杠打包,Linux unzip 解压成功但 exit=1,
 * 平台误判为失败报"解压失败 exit=1" —— 放宽为 exit<=1 视为成功。 */
export function isUnzipOk(code) {
...
export async function unzipAsync(zipPath, destDir) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (isUnzipOk(code)) resolve();
      else reject(new Error(`解压失败 exit=${code}${stderr ? ": " + stderr.trim().split("\n").pop() : ""}`));
    });
  });
...
/** 解析相对路径到 tools/ 或 data/ 根(仅这两个数据根合法),防目录穿越;非法返回 null。
 * rel 基于 DIRS.tools 的父级解析(兼容 "tools/<id>/x.zip" 与 "data/x" 两种写法),
 * 但结果必须落在 DIRS.tools 或 DIRS.data 之下 —— 2026-08-06 曾因根错位(用 tools 的父级=项目根)
 * 导致 /api/files 可写项目根任意文件(RCE),已收紧。 */
export function resolveWithinRoot(rel) {
  const dest = path.resolve(DIRS.tools, "..", rel);
  const roots = [path.resolve(DIRS.tools), path.resolve(DIRS.data)];
  if (!roots.some((r) => dest === r || dest.startsWith(r + path.sep))) return null;
  return dest;
}
